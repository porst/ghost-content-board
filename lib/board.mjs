...
//
// Parsing uses JSON.parse, not new Function()/eval() — Vercel's (and most
// other) Edge Runtimes disallow dynamic code generation from strings by
// default and throw "Code generation from strings disallowed for this
// context" the moment new Function() is called. The array literal is safe
// to parse as strict JSON because replaceArrayBlock always writes it via
// JSON.stringify, which only ever produces double-quoted keys, no trailing
// commas, and no comments — the one thing new Function() tolerated that
// JSON.parse doesn't.
